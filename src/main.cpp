...
#if defined(NDEBUG)
# error "Liberalcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0569f212db633a77e750bdd82990baeab8cefebb45f1e8f1fff91aa6004a3517");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Liberalcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Liberalcoin Signed Message:\n";
...
{
    // Liberalcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 300000 blocks, which will occur approximately every 2 years
    nSubsidy >>= (nHeight / 300000); // Liberalcoin: 300k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Liberalcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Liberalcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Liberalcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Liberalcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3a9f1dd8c638abbfc73c3e47ba0d2cd0908a605725753138335c5fb607a0ffaf");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Heart of Raven creates Liberalcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0475063a67662ddc1ee88514633cb6b152b25818a48673e56f8226298bd9c10a46d32efcc8ab60afc200fd09c858e09e8cf78425aee837bde91476433d642719d7") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521320863;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085707179;

        if (fTestNet)
        {
            block.nTime    = 1521320806;
            block.nNonce   = 386196901;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xe5034600ad4677e54c218a05782a06db52692d8b650c6a942540f07ee28d7897"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xba, 0xd4 }; // Liberalcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LiberalcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LiberalcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LiberalcoinMiner(CWallet *pwallet)
{
    printf("LiberalcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("liberalcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LiberalcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LiberalcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LiberalcoinMiner, pwallet));
}
